...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/prp-core/prp-loop/scripts/prp_loop.py "$ARGUMENTS"
```

Resume a halted or in-progress loop:

```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/prp-core/prp-loop/scripts/prp_loop.py --resume
```
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/prp-core/prp-loop/scripts/prp_loop.py "$ARGUMENTS" --until implement
```
